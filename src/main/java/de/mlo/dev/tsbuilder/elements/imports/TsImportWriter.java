...
public class TsImportWriter implements ITsImportWriter {
    public String write(TsContext context, TsImportList importList){
        Map<String, Set<TsImport>> map = flattenAndDistinctImports(importList);
     ...
     */
    private Map<String, Set<TsImport>> flattenAndDistinctImports(TsImportList importList){
        Map<String, Set<TsImport>> map = new LinkedHashMap<>();
        importList.forEach(tsImport -> distinct(map, tsImport));
        return map;
    }
